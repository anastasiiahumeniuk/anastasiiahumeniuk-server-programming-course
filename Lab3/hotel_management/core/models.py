...
class Booking(models.Model):
    ...
    def __str__(self):
        return f"Booking #{self.id} | Guest: {self.guest.first_name} {self.guest.last_name} | Room: {self.room.room_number} | Status: {self.booking_status} | Total: {self.total_price}"
...
class BookingService(models.Model):
    ...
    price_at_booking = models.DecimalField(max_digits=7, decimal_places=2, null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.service.service_name} × {self.quantity} | Booking #{self.booking.id} | Price at booking: {self.price_at_booking}"
